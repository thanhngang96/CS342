...
class List342
{
	friend ostream& operator<<(ostream& outStream, const List342<ItemType>& obj)
	{
		if (obj.head == NULL)
		{
			outStream << "Empty list!" << endl;
			return outStream;
		}

		Node * curNode = obj.head;
		while (curNode != NULL)
		{
			ItemType *insItem = curNode->data;
			outStream << *curNode->data;
...
public:
	...
	const List342& operator=(const List342& obj);
	List342<ItemType> operator+(const List342<ItemType> &rhs) const;
	List342<ItemType>& operator+=(const List342<ItemType> &rhs);
